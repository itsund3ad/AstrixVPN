...
class SessionPool:
    ...
    async def get_or_create(
        self, session_id: bytes, flags: int = 0, target: str = "", now: float = 0.0,
    ) -> Session:
        lock = self._lock_for(session_id)
        async with lock:
            if session_id in self._sessions:
                return self._sessions[session_id]
            s = Session(session_id=session_id, flags=flags, target=target, last_activity=now)
    ...
    async def drain_tx_limited_txn(self, session_id: bytes, max_frames: int) -> DrainSnapshot:
        ...
        async with lock:
            ...
            frames = [dq.popleft() for _ in range(take)]
    ...
    async def wait_for_activity(self, timeout: float = 0.010) -> bool:
