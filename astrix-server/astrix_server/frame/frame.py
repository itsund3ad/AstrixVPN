...
class Frame:
    ...
    def encoded_len(self) -> int:
        tb = self.target.encode("utf-8")
        return SessionIDLen + 8 + 1 + 1 + len(tb) + 4 + len(self.payload)
...
marshal_encoded_len = _marshal_size


def marshal_into(buf: bytearray, offset: int, frame: Frame) -> int:
    tb = frame.target.encode("utf-8")
    tlen = len(tb)
    off = offset
    ...
    if tlen:
        buf[off : off + tlen] = tb
        off += tlen
    struct.pack_into(">I", buf, off, len(frame.payload))
    off += 4
    return off - offset
...
def unmarshal(data: bytes) -> tuple[Frame, int]:
    ...
    off += plen
    return Frame(session_id=sid, seq=seq, flags=flags, target=target, payload=payload), off
